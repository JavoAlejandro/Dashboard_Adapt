...
| `js/gps.js` | Modify | `initGPSMap()`: `#map-gps-wrap` (Exposición map) display toggle restored to normal — an earlier commit mistakenly hid this instead of Congestión's own wrap, since both share the `.map-gps-wrap` CSS class under different ids (`#map-gps-wrap` vs `#map-cong-wrap`). Corrected 2026-07-22. |
| `js/congestion.js` | Modify | `congRenderFootprint()`: `#map-cong-wrap` display toggle intentionally commented out — kept hidden pending a later revision (this was the element actually meant to stay hidden). Corrected 2026-07-22. |
